...
<<<<<<< HEAD
    ...
    const filtered = filteredData.map((row) => ({
      "Full Name": toTitleCase(`${row.firstName || ""} ${row.lastName || ""}`.trim()),
...
<<<<<<< HEAD
    ...
    const headers = ["Full Name", "Father Name", "Track", "Village", "Student Mobile"];

    const tableHeaders = headers.map((title) => ({
      content: title,
      styles: { fillColor: [220, 220, 220], halign: "center" },
    }));
